...
public:
    ...
    bool levels() override
    {
        ...
        srand(time(0));
        RenderWindow window(VideoMode(800, 800), "Level1");
        //Using sfml functions to writev text of live variable on screen
            ...
            if (!pause || newGame)
            {
                ...
                for (int i = 0; i < numSmallBoxes; ++i)
                {
                    ...
                    //if (powerUpActive && powerUpTimer.getElapsedTime().asSeconds() >= powerUpDuration)
                     if (powerUpTimer.getElapsedTime().asSeconds() >= 10.f)
                    {
                         //cout << "Speed check\n";
                        for (int j = 0; j < numSmallBoxes; ++j)
                        {
